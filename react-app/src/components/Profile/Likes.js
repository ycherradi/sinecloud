...
const dispatch = useDispatch();
  ...
  const likedSongs = likes.map((like) => songs[like])
  ...
  return (
    <div className='row'>
      <div className='column liked__div--outer'>
                                ...
                                <div className="item__liked">
                                  <div className="image__container" >
                                      <div className="image1" onClick={() => {onClick(song.id);}}>
                                        ...
                                        {likes?.includes(song?.id) ? <button id={`${song?.id}`} className='liked-1' onClick={(e) => handleRemoveLike(e, song?.id)}></button> : <button id={`${song?.id}`} className='like-1' onClick={(e) => handleAddLike(e, song?.id)}></button>}
                                      </div>
        ...
        {currentSong && <ReactJkMusicPlayer 
                {...options}
